...
class MathAgent {
    ...
    process(inputSnapshot) {
        ...
        if (isMidBand && isPowderBase) {
            ...
            phase1.techTips = [
                "⚠️ СВІЖІСТЬ СУМІШІ: Пудра активна 30-40 хв. Готуйте суміш окремо для кожної зони голови (мінімум 4 заміси).",
                "⚠️ ТОВЩИНА ПАСОМ: Працюйте тонким зрізом до 1 см.",
                "⚠️ ТАЙМІНГ КОРЕНЯ: Наносити на корінь одразу після завершення нанесення на довжину (або за візуальним контролем).",
                "🧼 ЕТАП ОЧИЩЕННЯ: Змити пудру ШГО (2 рази). Обов'язково нанести ліпідну маску на 5 хв для наповнення структури ПЕРЕД тонуванням."
            ];
            phases.push(phase1);
            timing = 90 + tMod;
            ...
            phase2.techTips = [
                "🎨 ФІКСАЦІЯ КОЛЬОРУ: Після тонування використовувати ТІЛЬКИ кондиціонер з низьким pH. Маску НЕ використовувати (вона вимиває пігмент).",
                "🎨 ПОРАДА ДЛЯ СИВИНИ/.11: Якщо щільності попелу на сивині недостатньо, повторіть нанесення суміші (Фарба+Оксид 1:1) без шампуню."
            ];
            phases.push(phase2);

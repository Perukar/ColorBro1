...
- Правило DAMAGE / POROSITY / ELASTICITY: знайдено. high damage/porosity не має давати automatic APPROVED; root damaged + root lift/powder/Special Blond covered by UI field + guard + tests; Special Blond + high porosity covered by UI field + guard + tests; brittle блокує освітлення; warning-only не достатній для high-risk lift.
...
- Пошкоджене волосся / еластичність / пористість: DAMAGE / POROSITY / ELASTICITY is partially covered by existing guards/tests. Root damaged + root lift/powder/Special Blond covered by UI field + guard + tests. Low elasticity covered by UI field + guard + tests. Special Blond + high porosity covered by UI field + guard + tests. Gap: немає повної матриці по root/length/ends, length damaged / brittle lift, high oxidizer, powder для length damage, toning, darkening, multi-zone damage conflict, brand-specific constraints.
...
|---|---|---|---|---|---|
| 1 | length damaged / brittle lift | Довжина може мати окремий стан пошкодження, який не має автоматично успадковувати root guard | Length damaged / brittle + lift/powder -> no automatic approved unsafe recipe | Так | Так |
| 2 | high oxidizer + damaged hair | Високий оксид на пошкодженому полотні може створити ризик навіть без root powder scenario | Damaged hair + high oxidizer -> no automatic approved unsafe recipe | Так | Так |
...
| 4 | multi-zone damage conflict / різнозонне полотно | Різні зони можуть мати різну косметичну історію, пористість і реакцію | Multi-zone history 3-5 years + different porosity -> no automatic approved unified recipe | Так | Так |

...
- Правило DAMAGE / POROSITY / ELASTICITY: знайдено. high damage/porosity не має давати automatic APPROVED; Special Blond + high porosity covered by UI field + guard + tests; brittle блокує освітлення; warning-only не достатній для high-risk lift.
...
- Пошкоджене волосся / еластичність / пористість: DAMAGE / POROSITY / ELASTICITY is partially covered by existing guards/tests. Low elasticity covered by UI field + guard + tests. Special Blond + high porosity covered by UI field + guard + tests. Gap: немає повної матриці по root/length/ends, high oxidizer, powder, toning, darkening, multi-zone damage conflict, root damaged powder, length brittle lift.
...
|---|---|---|---|---|---|
| 1 | root/length damage matrix або root damaged powder | Потреба у повній матриці пошкоджень, особливо для пошкодженого кореня/довжини та порошку | Root/length damage + powder/lift combinations -> no automatic approved unsafe recipe | Так | Так |

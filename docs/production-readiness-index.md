...
| `test_www_brand_matrix_import_contract.js` | Brand matrix admin/import readiness (diagnostic-only): payload shape + canonical entry validation; import-ready never activates production (hasBrandRuleMatrix stays false; calculateProtocol unchanged); pure; TEST_* fixtures only (16 groups) |

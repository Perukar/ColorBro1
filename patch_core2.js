...
// We will replace MathAgent
const startMarker = '// --- MATH AGENT ---';
const endMarker = '// --- MASTER NODE ---';
...
class MathAgent {
    ...
    calculateLength(snapshot, massDist) {
        ...
        let diagnostics = [];
        if (lenRec && (lenRec.process === ProcessType.POWDER || lenRec.process === ProcessType.DECAPITATION) && [HairCondition.POROUS, HairCondition.DAMAGED].includes(condition)) {
            if (lengthLevel >= 9) {
                lenRec = { ...lenRec, process: "Емульгація", dye: "Відпрацьована суміш", ox: "-", ratio: "-", mass: 0 };
                diagnostics.push("ПРАВИЛО 2: Пориста довжина 9-10 УГТ — свіжа пудра заборонена. Призначено емульгацію.");
            } else {
...
console.log("core.js updated with Chemical Rules 1, 2, 3, 4.");
